...
export default function NavBar() {
  ...
  return (
    <nav className="bg-[#0a1628] px-6 h-23 flex items-center justify-between relative">
      ...
      <button
        className="cursor-pointer flex items-center gap-3 no-underline"
        onClick={menuTravel}>
      ...
      <div className="flex items-center gap-4">
        <div className="text-right hidden sm:block">
          ...
          <div className="text-[13px] text-[#c8a84b] tracking-wider uppercase">
            {role}
          </div>
        </div>
        ...
        <div className="relative" ref={menuRef}>
          <button
            onClick={toggleMenu}
            className="w-12 h-12 rounded-full bg-[#006BB6] border-2 border-[#c8a84b] flex items-center justify-center font-sans font-bold text-xl text-white cursor-pointer shrink-0 hover:scale-105 transition-transform focus:outline-none select-none">
            {siglas.toUpperCase()}
          </button>
          ...
          {isOpen && (
            <div className="absolute top-[calc(100%+10px)] right-0 bg-[#112240] border border-white/10 rounded-xl overflow-hidden min-w-[250px] shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              <div className="p-4 pb-2.5 border-b border-white/10 sm:hidden">
                ...
                <div className="text-sm text-[#c8a84b] mt-0.5 uppercase tracking-wider">
                  {role}
                </div>
              </div>
              ...
              <button
                onClick={logout}
                className="flex items-center gap-2.5 px-4 py-3 text-[#f08080] text-base cursor-pointer hover:bg-[#c0392b]/15 hover:text-[#f08080] transition-colors border-none bg-transparent w-full text-left">
