      ...
      <div className="text-xl">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-5">
          <input
            ...
            className="border border-[#d0dcea] rounded-lg px-4 py-2 flex-1"
          />

          <select
            value={filtroFecha}
            onChange={(e) => setFiltroFecha(e.target.value)}
            className="border border-[#d0dcea] rounded-lg px-4 py-2">
            <option value="todos">Todos los partidos</option>
            <option value="proximos">Próximos partidos</option>
            <option value="finalizados">Partidos finalizados</option>
          </select>
        </div>
          ...
          {data.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {partidosFiltrados.map((partido) => {
                ...
                const sectoresPartido = dataSectores.filter(
                  (sector) =>
                    sector.identificadorPartido === partido.identificador,
                )

                return (
                  <div
                    key={partido.identificador}
                    className="bg-white border border-[#d0dcea] rounded-2xl overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                    <div className="relative bg-[#0a1628] h-52 flex flex-col items-center justify-center">
                      ...
                      <div className="relative flex items-center gap-6">
                        <img
                          src={partido.banderaEquipoLocal}
                        ...
                        <img
                          src={partido.banderaEquipoVisitante}
                    ...
                    <div className="p-5">
                      <div
                        className={`gap-y-6 ${sectoresPartido.length > 0 ? 'flex flex-col' : ''}`}>
                      ...
                      <div className="flex gap-2 mt-6 flex-col">
                        <button
                          onClick={() => {
                            setIdentificadorPartido(partido.identificador)
                            setEstadio(partido.identificadorEstadio)
                            setOpenUpdate(true)
                          }}
                          className="cursor-pointer flex-1 bg-[#0a1628] text-[#c8a84b] py-2.5 rounded-lg font-medium hover:bg-[#13203a] transition">
                        ...
                        <button
                          ...
                          className="flex-1 inline-flex items-center justify-center font-medium py-2.5 rounded-lg cursor-pointer transition-all bg-transparent border border-[#d0dcea] text-[#0a1628] hover:bg-[#dce6f5]">
                        ...
                        <button
                          ...
                          className="cursor-pointer px-4 py-2.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition">
