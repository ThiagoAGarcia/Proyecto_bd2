...
public static class AdministradorEndpoints
{
    public static void MapAdministradorEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/administrator/events", async (IConfiguration config, HttpContext context) =>
        {
            ...
            var MailPerfil = Normalizar.NormalizarMethod(reader.GetString("MailPerfil"));
            var NombrePais = reader.GetString("NombrePais");

            await using var stadiumCommand = connection.CreateCommand();
            stadiumCommand.CommandText = """
                SELECT 'identificador', 'imagen', 'nombre', 'direccionLocalidad', 'direccionCalle', 'direccionNumero'
                FROM 'estadio'
                WHERRE 'nombrePais' = @nombrePais;
            """;
            
            stadiumCommand.Parameters.AddWithValue("@nombrePais", NombrePais);

            await using var stadiumReader = await stadiumCommand.ExecuteReaderAsync();
            if (!await stadiumReader.ReadAsync())
            {
                return Results.NotFound();
            }
            ...
            return Results.Ok(new
            {
                identificador = Normalizar.NormalizarMethod(reader.GetString("identificador")),
                imagen = reader.GetString("imagen"),
                nombre = Normalizar.NormalizarMethod(reader.GetString("nombre")),
                direccionLocalidad = Normalizar.NormalizarMethod(reader.GetString("direccionLocalidad")),
                direccionCalle = Normalizar.NormalizarMethod(reader.GetString("direccionCalle")),
                direccionNumero = Normalizar.NormalizarMethod(reader.GetString("direccionNumero")),
                NombrePais = reader.GetString("NombrePais")
            });
